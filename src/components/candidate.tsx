...
import { useDeleteCandidateById } from '@/hooks/queryHook'
...
const Candidate = (props: CandidateProps) => {
  ...
  const deleteCandidate = useDeleteCandidateById()
  return (
    <article className="border border-gray-300 rounded-lg p-4 shadow-md flex-col w-[300px]">
      ...
      <button className="mt-4 w-full bg-yellow-500 hover:bg-yellow-600 text-white font-semibold py-2 px-4 rounded">
        <Link to="/form/$id" params={{ id }}>
          {' '}
          Editar{' '}
        </Link>
      </button>
      ...
      <button
        className="mt-4 w-full bg-red-500 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded"
        onClick={() => {
